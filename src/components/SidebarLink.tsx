  ...
  return (
    <NavLink.Root className={cn('py-2 relative', isActive ? activeStyles : '')}>
      <NavLink.Link
        href="/"
        className={cn(
          'px-6 py-1 flex items-center gap-4 border-x-2 border-x-transparent group focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 focus:ring-offset-slate-800',
          isActive ? 'border-l-teal-500' : 'border-l-transparent'
        )}
      >
        <NavLink.Icon
          icon={icon}
          className={cn(
            'h-6 w-6 group-hover:text-teal-500 group-focus:text-teal-500 transition-colors duration-200',
